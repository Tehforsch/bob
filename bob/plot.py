...
def runPlot(function: Callable[[plt.axes, SimulationSet], None], sims: SimulationSet, args: argparse.Namespace) -> None:
    function(plt, sims)
    plt.savefig(Path(sims.folder, config.picFolder, function.__name__), dpi=config.dpi)
    if args.showFigures:
        plt.show()
