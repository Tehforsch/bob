...
from bob.config import (
    outputFolderIdentifier,
    initialSnapIdentifier,
    snapshotFileBaseIdentifier,
)
...
class SimulationSet(list):
    def __init__(self, folder: Path, sims: Iterable[Simulation]) -> None:
        ...
        self.variedParams = set(
            k
            for k in self[0].params
            if self.doesVary(k) and not k in self.derivedParams
        )
    ...
    def quotient(self, parameters: List[str]) -> List[Any]:
        ...
        getConfiguration = lambda sim: tuple(
            (k, sim.params[k]) for k in remainingVariedParams
        )
    ...
    def getNiceSimName(self, sim: Simulation) -> str:
        assert sim in self
        result = ",".join(
            getNiceParamName(k, sim.params[k])
            for k in self.variedParams
            if getNiceParamName(k, sim.params[k]) != ""
        )
        return result
    ...
    def getNiceSubsetName(self, params: List[str], simSet: "SimulationSet") -> str:
        ...
        result = ",".join(
            getNiceParamName(k, simSet[0].params[k])
            for k in params
            if getNiceParamName(k, simSet[0].params[k]) != ""
        )
        return result
...
@toList
def getProductSubstitutions(
    subst: Dict[Any, Any], cartesianOptions: Union[bool, List[List[str]]]
    ...
    if isinstance(cartesianOptions, List):
        for combinedParams in cartesianOptions:
            ...
            ), "Unequal parameter list lengths for params: {}".format(cartesianOptions)
...
def getAllSubstitutions(args: argparse.Namespace) -> Dict[str, Any]:
    if args.create:
        ...
        simNames = list(
            f
            for f in args.simFolder.glob("*")
            if (not f.name in config.specialFolders) and f.is_dir()
        )
...
def copyInitialSnapshot(folder: Path, sim: Simulation, initialSnap: Path) -> None:
    ...
    shutil.copyfile(
        Path(folder, initialSnapshot), Path(outputFolder, nameFirstSnapshot)
    )


def createSimulation(
    args: argparse.Namespace, name: str, d: Dict[str, Any]
) -> Simulation:
...
def createSimsFromFolder(args: argparse.Namespace) -> SimulationSet:
    ...
    if cartesianOptions:
        ...
        dicts = [
            dict((k, v[i]) for (k, v) in allSubstitutions.items())
    ...
    sims = SimulationSet(
        args.simFolder,
        (
            createSimulation(args, name, d)
            for (name, d) in zip(names, dicts)
            if args.select is None or name in args.select
        ),
    )
    return sims
