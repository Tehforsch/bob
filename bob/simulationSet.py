...
def fromFolder(createSimulations: bool, args: argparse.Namespace) -> SimulationSet:
    allSubstitutions = readSubstitutionsFile(args.inputFolder)
    if allSubstitutions == {}:
        return SimulationSet([Simulation(createSimulations, "sim", args.inputFolder, args.outputFolder, {})])
    ...
    return SimulationSet(Simulation(createSimulations, name, args.inputFolder, Path(args.outputFolder, name), d) for (name, d) in zip(names, dicts))
